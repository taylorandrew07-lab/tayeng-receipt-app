...
**Supersedes `docs/reconciliation-plan.md` Steps 5–11 and cancels Step 4c.**
Steps 1–4 of that document are **built and applied** (migrations `0014_reconciliation_settings.sql`, `0015_charge_identity.sql`, `0016_match_durability.sql`, `0017_reconciliation_views.sql`) and are not redone here. Note that the old plan's migration *names and numbers are wrong* — it calls the backup table `rsm_backup_pre_0015`; the real table is `rsm_backup_pre_0016` (`0016_match_durability.sql:14-15`). Where the old plan and this document disagree, **this document wins**.

Everything below was written against the files as they exist today. File:line citations are real.
...
There are only two kinds of thing on the list:

1. **A charge with no receipt.** A real purchase that appeared on a statement, that we do not yet have paperwork for.
2. **A receipt with no charge.** Paperwork we have, that does not appear on any statement (paid cash, paid on another card, or the statement covering it has not been uploaded).

Both kinds are *open items*. The screen shows one number — **"18 open · TTD 11,648.69"** — and that number only ever goes down, and only ever for a stated reason.

An item leaves the list exactly four ways, and every one of them is recorded, reversible, and attributed to either Andrew or the machine:
...
| **No receipt needed** — bank fee, interest, a payment to the card | `charges.no_receipt_expected` (`0015_charge_identity.sql:50`) + `closed_reason` |
| **No charge expected** — cash, personal, accountant handles it, written off | `receipts.no_charge_expected` (**new** — this column does not exist and is the reason the count can never currently reach zero) |

**A charge is a real-world purchase, not a line on a statement.** This is already true in the database: `charges` (`0015:41-53`) gives each real charge one identity that survives across overlapping statements. Live data: 90 statement lines collapse to **64 real charges**; 24 of them appear on two or more statements; one appears on three. Raw line totals read TTD 42,920.49; the real figure is TTD 30,583.05 — the list was inflated by TTD 12,337.44 (29%). Fifteen lines were reported "missing receipt" when the charge already had one, and twelve of those had already been sent to the accountant.
...
- No `?month=YYYY-MM` parameter is ever built. No month picker. No `dateFrom`/`dateTo` scoping on the close-out surface.
- Migration `0019` issues `drop view if exists reconciliation_months;`. That view (`0017_reconciliation_views.sql:126-146`) is applied but **has zero consumers** — grep across `app/`, `lib/`, `components/` returns no query against it, and no code path was ever written that depends on it. Dropping it costs one line and breaks nothing. It is dropped rather than commented as deprecated because a comment is exactly as visible as nothing to someone grepping for `month` in six months' time.
- The `/reports` monthly report and `receipts.month_key` are **not touched**. That report is a separate deliverable Andrew already hands to his accountant and it keeps working exactly as it does now.

### Justifying this against *both* things Andrew said

He asked for a month view earlier. He now says *"take away this sort of monthly rules that we've been setting up and make it more about closing out all the outstanding receipts from the statements."* These are not equally weighted opinions to be split down the middle, because the month axis fails on correctness, not taste:

1. **The month a receipt is filed under is the month it was uploaded, not the month it was spent.** `receipts.month_key` is set from upload time (migration `0004_duplicates_and_upload_month.sql`; `app/api/receipts/extract/route.ts:132` passes `uploadedAt: new Date(receipt.created_at)`). **62 of 82 receipts are filed under a `month_key` that differs from their `receipt_date`.** All four statements were uploaded on one day. A "July" view would show every May charge.
2. **A month filter hides the receipt that closes the charge.** The candidate window is deliberately 60 days before and 15 days after (`0014_reconciliation_settings.sql:16-19`) because the receipt for a 30 May charge is routinely dated in April or June. Scoping a *matching* surface to a calendar month removes the correct answer from the screen. This is a correctness argument and it is decisive.
3. **The app already committed to outstanding-first and it worked.** `app/api/reports/generate/route.ts:36-37` defaults the reimbursable report to `scope=outstanding` — `reimbursable = true AND paid = false`, **no month filter** — with the code comment explaining that `month_key` is the upload month. That change is applied and correct. Keeping a month lens on reconciliation would mean two competing definitions of "done" in the same app.

**Dates do not disappear. They stop being a scope and become a sort and a label.** The list sorts oldest-open-first so the stalest debt surfaces. Rows carry an age badge ("open 94 days"). The close-out PDF prints the real period it covers, derived from `statement_coverage.effective_start/effective_end` (`0014:51-65`) — the first ever consumer of that view — and where the period was inferred rather than read off the document, it says so (see §2, `statements.period_inferred`; two live statements parsed with `period_start IS NULL`).

---

# 2. SCHEMA CHANGES

Two migrations. `0018` exists solely because Postgres cannot *use* an enum value in the transaction that *adds* it, and the Supabase CLI wraps each migration file in one transaction.
...
`rule_type` today is `('vendor_category','last4_card','vendor_payment')` (`0001_initial_schema.sql:28-30`). `vendor_payment` exists and is never written or read — it stays untouched.
...
Organised in nine labelled sections. Sections A–D and H are additive. Sections E, F and G **change existing rows** and each takes a backup or is scoped so that no human decision can be overwritten.
...
The missing half of the checklist. 43 receipts have no statement line; **27 of those are unsent, worth TTD 11,648.69**, and most will never have a statement line. Without a way to close a receipt, the open count can never reach zero and the whole model fails.
...
> **No charge-side fingerprint is created.** A charge's `description`, `amount`, `currency` and `card_last4` *are* its `charge_key` and are immutable for a given charge by construction (`0015:23-30`); `anchor_date` is the only mutable field and it can only move earlier (`0015:124`). A charge fingerprint would be a second source of truth with nothing to say.
...
returns text language sql immutable as $$ /* full body in §4.2 */ $$;
...
begin
  ...
  select count(*) into unflagged from charges
    where not no_receipt_expected and id in (select id from charges_backup_pre_0019 where no_receipt_expected);
...
-- Expect 'fee' to contain OVERLIMIT FEE 45.00. Expect no 'purchase' charge to
-- have vanished: the count of line_kind='purchase' must equal 64 minus the
-- fee/interest/payment/summary rows.
...
### Section G — `assign_charge()` rewritten (**DATA-CHANGING behaviour, no rows touched by the DDL**)
...
2. **Apply Tier 4b** (see §4.2): a description matching the *ambiguous* payment patterns is upgraded to `card_payment` **only when `new.direction = 'credit'`**.
...
5. **On charge creation** (`0015:113-122`) set `line_kind`, `direction`, `kind_source`, and derive the two legacy booleans so the existing view keeps working: `no_receipt_expected := v_kind not in ('purchase','unknown')` and `fee_auto_flagged := (v_source <> 'user')`.

`unknown` is chased. Failing safe means showing noise; showing noise is the cheaper error.

### Section H — views (**ADDITIVE to the read model; replaces two views nothing reads yet**)
...
`charge_reconciliation` — everything at `0017:17-82` is kept; these columns are appended to the outer select and the `state` CASE is reordered:
...
> **The reorder is a real bug fix.** Today `no_receipt_expected` is evaluated *first* (`0017:74`), ahead of `already_sent` and `already_matched`. A charge wrongly flagged as a fee that in fact has a confirmed, already-sent receipt renders as "no receipt expected" and the receipt disappears from view entirely. Only *confirmed* evidence outranks the flag; a stale pending suggestion does not. This affects zero rows in current live data, which is exactly why it should be fixed now rather than discovered later.

`orphan_receipts` — everything at `0017:96-111` is kept; the filter is widened and columns appended:
...
> **Widening `status` is the other real bug fix.** As applied, a receipt still sitting in `needs_review` is invisible to reconciliation entirely — and those are precisely "receipts that have not been processed since the last time." They are surfaced with a `needs_review` badge so they read as *needing attention*, not as ready-to-match.
...
update statements s set period_inferred = true
where exists (select 1 from statements_backup) is null;   -- see note
```

> The backfill for `period_inferred` cannot be derived after the fact — `0014` overwrote the NULLs without a record. Set it to `true` for the two statements Andrew identifies at Step 11, or accept `false` and let the flag be correct from the next upload onward. **Do not guess.** The plan's choice: leave existing rows `false`, and have Step 11's UI show "period not verified" for any statement where `printed_closing_balance is null` — which is true for all four existing statements and is the honest signal.
...
- **No `reconciliation_runs` table.** Its own design concedes the watermark is advisory and that correctness never depends on it. A table correctness never depends on exists only to power run-history screens Andrew did not ask for. `decided_via` + the two scalars deliver attribution and the "new since" lens with zero new tables.
- **No `open_items` or `ledger_summary` view.** `charge_reconciliation` and `orphan_receipts` now both carry `is_open`; the header count is two `count(*)`s in `lib/reconciliation/`. Two views nothing reads do not need a third and fourth wrapped around them.
- **No `vendor_aliases` table.** `learning_rules` already has `unique(user_id, rule_type, pattern)` (`0001:260`), an `action jsonb` (`0001:256`), a never-incremented `hit_count` (`0001:257`) purpose-built for ranking, and the index `learning_rules_user_idx (user_id, rule_type)` (`0001:262`).
- **No cached scores, no `ALGO_VERSION`, no `last_matched_at` cursor.** See §4.4.
- **`pg_trgm` is not enabled and nothing may assume it.** Grep for `create extension` across `supabase/migrations/` returns zero hits.
- **`vendors` is not used as the canonical alias target.** It is defined (`0001:107-118`) with a perfect-looking `unique(user_id, normalized_name)`, but **no code ever writes it** — the only reference anywhere is `vendor_id` in a select list at `app/(app)/receipts/page.tsx:38`. It is a dead table.

---

# 3. THE STEPS

Eleven steps. **The app works after every one.** Andrew sees the first real payoff at Step 7 and the count can reach zero at Step 8.

---

## Step 5 — Safety rails (half a day, small but visible)

Nothing else may ship first. The moment a persistent count goes on screen, the app is promising that **the number only ever changes for a stated reason** — and `deleteStatement` currently breaks that promise silently.

**Changed:**
- `lib/statements/actions.ts:7-26` — `deleteStatement`. Today it discards `{ error }` on both the storage `remove()` (`:18`) and the `delete()` (`:21`), and carries the comment *"statement_transactions + matches cascade-delete via FK"* at `:20` which has been **false since `0016_match_durability.sql:31-36` changed `statement_transaction_id` to `ON DELETE SET NULL`.** Rewrite to: count confirmed matches on this statement's charges first; if > 0, return `{ ok:false, message:"This statement has N confirmed receipt matches. Deleting it will not delete them, but the charges will leave your list. Type DELETE to confirm." }`; check `{ error }` on both calls and surface it.
- `components/delete-button.tsx` — accept the typed result and show it.

**New:**
- `package.json` — add `vitest` devDependency and `"test": "vitest run"`. There is no test runner and no `*.test.*` file anywhere in the repo, and `lib/matching/match.ts` is about to be substantially rewritten.
- `lib/reconciliation/paginate.ts` — `fetchAll<T>(build: (from:number,to:number) => PostgrestFilterBuilder<T>): Promise<T[]>`, 1000-row pages. Every unbounded query in the codebase (`lib/matching/actions.ts:22,30,43`; `app/(app)/matching/page.tsx:40,49,100,107`; `app/api/reports/statement/route.ts:49,59`; `lib/reports/append-receipts.ts:62`) routes through it as it is touched. **Load-bearing for §4.1:** building the IDF corpus from a truncated 1,000-row PostgREST page would silently change every vendor score.
...
**Verify:** try to delete a statement with confirmed matches; the dialog names the count. `npm test` runs and reports 0 tests.

---

## Step 6 — Migrations `0018` and `0019` (one day, invisible)

Apply §2. Run every verification query in §2 and paste the output into the PR.

**After this step Andrew can:** nothing new — but the fee/payment lines are already classified and closed, the receipt-side closure columns exist, and the two views are correct. This is the only invisible step and it is one day.
**Verify:** the three `select` statements in §2.2 sections F and H; `select count(*) from charge_reconciliation where is_open` should be roughly 64 minus fees minus already-matched; `select count(*) from orphan_receipts where is_open` should be **27**.

---

## Step 7 — The close-out list at `/reconcile` (read-only) — **FIRST VISIBLE WIN**

One merged list across every statement, counted once, correct totals, bank noise already gone. No buttons yet beyond navigation. This is the screen that replaces the statement picker.
...
- `app/(app)/reconcile/page.tsx` — server component. Header: **"18 open · TTD 11,648.69"** over a progress bar reading *"closed 46 of 64"*. Underneath, the completeness line (Step 11 fills it; until then it reads *"statement totals not read — can't verify this list is complete"*). Then filter chips: **All open** (default) · Needs a receipt · Suggested (n) · Receipt with no statement line · Closed. Then a single flat list, **oldest first, no statement grouping, no month grouping**. Each row: bold right-aligned amount, description/vendor, date, age badge ("open 94 days"), an `on 3 statements · counted once` chip where `copies > 1`, and a `NEW` dot where `charge_created_at > last_reconcile_seen_at`.
- At the foot of the list, two collapsed strips, always on screen and never a filter: **"🏦 Bank charges — nothing to chase · 5 · TTD 312.00"** and **"Payments & credits — not spending · 2 · −TTD 5,000.00"**.
- `components/reconcile/open-row.tsx`, `components/reconcile/summary-header.tsx`.

**Changed:**
- `components/app-shell/nav.ts:19` — `{ href:"/matching", label:"Matching", icon:"🔗" }` becomes `{ href:"/reconcile", label:"Close-Out", icon:"✅" }`. `/matching` stays reachable and working; it is retired at Step 14.

**After this step Andrew can:** open one screen and see, for the first time, the true number of outstanding items and the true total across all four statements — TTD 30,583.05 rather than 42,920.49 — with the 24 carried-over charges counted once and the bank fees already off the list.
**Verify:** the header total matches `select sum(amount) from charge_reconciliation where counts_as_spend`. The 15 lines that were falsely "missing receipt" no longer appear as missing. The 27 unsent orphan receipts appear.
...
`linkChargeToReceipt` pre-checks both partial unique indexes (`rsm_unique_confirmed_charge`, `0016:157-158`; `rsm_unique_confirmed_receipt`, `0013:14-16`) so the user sees a sentence rather than `23505`, then upserts on `onConflict: "receipt_id,charge_id"` (`rsm_unique_pair`, `0016:160-161`). **It passes `statement_transaction_id: canonicalTxnId` as well as `charge_id`** — the `rsm_sync_charge` BEFORE-INSERT trigger (`0016:52-68`) reads `charge_id` and all four `snap_*` provenance columns off the transaction, and passing only `charge_id` would leave the snapshot null. It sets `decided_via` and `rejected_at: null`.

**`unlinkCharge` and `rejectSuggestion` set `rejected_at = now()` and never delete.** This is the fix `0016:26-27` was explicitly written for — *"Set by rejectMatch instead of deleting, so a rejected pairing is never resurrected by a later run"* — and which `lib/matching/actions.ts:202` still has not applied. They also stamp `rejected_fingerprint` from the receipt's current `match_fingerprint`.

**Changed:**
- `components/toast.tsx:14-17` — `toast(message, variant, action?: { label, onAction })`; render the action button beside the dismiss control at `:64-71` and raise `ttl` to 8000 when an action is present. ~15 lines. **This is the single most important safety valve on the whole surface.**
- `app/(app)/reconcile/page.tsx` — every row gets one primary action that changes by state (`Attach` → `Yes, that's it` → `···`) and a `···` menu with the close reasons **in Andrew's words**: *Bank charge, never chase · Paid cash, no receipt exists · Personal, not a business expense · Accountant will handle it · Written off*. Orphan-receipt rows get the same menu minus the bank options.
- New `app/(app)/reconcile/closed/page.tsx` — the closed list, grouped by reason with counts, each row showing the reason, the date, whether Andrew or the machine decided it (`closed_by` / `decided_via`), and a **Reopen** button. An **"Auto-closed since you last looked (n)"** tray pinned to the top.
- `lib/matching/actions.ts` — delete the hand-written `is_matched` updates at `:88-92`, `:188-193` and `:204-207`. The `rsm_sync_is_matched` trigger (`0016:74-93`) derives it; writing it by hand is now a source of drift.
...
**Verify:** close one fee charge and one orphan receipt; the header decrements; both appear under `/reconcile/closed` with a reason and a Reopen button; Reopen restores the count.

---

## Step 9 — Vendor identity and scoring hardening (pure functions, fully tested)

This must land **before** the global pass, or a defective matcher gets pointed at every charge at once. Full specification in **§4.1** (aliasing) and **§4.3** (scoring gate).

**New:**
- `lib/matching/vendor-alias.ts` — `statementZone`, `receiptTokens`, `zoneKey`, `buildIdf`, `vendorScoreV2`. Pure, no I/O, no Supabase import.
- `lib/matching/vendor-alias.test.ts` — the **30-row test table in §4.1.4 verbatim** as fixtures.
- `lib/matching/match.test.ts` — the five known-bad live pairs as fixtures: the 224.70 (01 Jun) / 222.32 (03 Jun) cross-match that auto-confirmed **both** above 85, and the four July receipts pulled backwards onto April–June charges at gaps of 43, 50 and 71 days.
...
**Changed:**
- `lib/matching/match.ts` — replace `vendorScore` (`:63-71`); reweight `scorePair` (`:97`) from `a·0.50 + d·0.20 + v·0.20 + card` to **`a·0.45 + v·0.35 + d·0.15 + card`**; card mismatch becomes **disqualifying** rather than `−0.15` (`:94`); null date drops from `0.3` to `0.25` (`:86`); `MatchTxn`/`MatchReceipt` (`:3-17`) gain `currency` and `charge_id`; `STRONG_CONFIDENCE` rises 75 → 82 and becomes **conjunctive plus a uniqueness margin**; `candidates.sort` (`:124`) gains deterministic tiebreakers; an exact-amount pre-pass runs before the greedy loop.
- `app/api/receipts/extract/route.ts:121` — `.select("*")` becomes `.select("*").in("rule_type", ["vendor_category","last4_card"])`. It is unfiltered and unpaginated and runs on every receipt extraction; adding `vendor_alias` and `line_kind` rows would make it strictly worse. `classify()` reads only those two types (`lib/classification/classify.ts:71-73`, `:104-109`).
- `lib/types.ts:25` — `RuleType` gains `"vendor_alias" | "line_kind"`.

**After this step Andrew can:** nothing new on screen, but the next time matching runs it will know that `SCL TRINIDAD LTD SAN JUAN` is `SCL (Trinidad) Limited` and will refuse to guess between 224.70 and 222.32.
...
Full specification in **§4.4**.

**New:** `lib/reconciliation/run.ts` — `runCloseOut(): Promise<RunSummary>`.
...
- **Candidate windowing** per charge from `receipt_window_days_before` (60) and `receipt_window_days_after` (15) (`0014:16-19`) — read by no code today, which is exactly why four July receipts were pulled backwards. Receipts already confirmed or already sent are **exempt**, per the column comment at `0014:25-26`. The run **reports** how many receipts fell outside the window; a silent window is the failure mode this design exists to prevent.
...
**Changed:**
- `lib/matching/actions.ts:197-210` — `rejectMatch` stops deleting. **This must ship in the same commit as the sweep.** The current delete at `:65-69` is `.delete().in("statement_transaction_id", txnIds).eq("confirmed", false)`, and a rejected row *has* `confirmed = false` — so fixing `rejectMatch` alone would leave the very next `runMatching` deleting the rejection anyway.
...
- `app/(app)/reconcile/page.tsx` — a single primary **"Find matches"** button using `useActionState`, returning an inline banner: *"scanned 18 open items · 6 suggestions · 2 bank charges auto-closed · 3 receipts outside the date window were not considered."*
...
**Verify:** run it twice with no new data; the second run produces **byte-identical rows including unchanged `created_at`**. Reject a suggestion, re-run, confirm it does not come back.

---

## Step 11 — Noise: the parser stops deleting, and Andrew can correct it

Full specification in **§4.2**.
...
- `lib/extraction/parse-statement.ts:34` — **delete this line.** It currently reads *"Only include purchases/charges (money spent). EXCLUDE payments to the card, refunds, credits, interest, and fees unless they are clearly purchases."* Replace with *"Include EVERY line in the transaction table. Do not omit anything. Label each one with `kind` and `direction` instead."* Full replacement prompt in §4.2.4.
- `app/api/statements/parse/route.ts:109-119` — map `amount: Math.abs(t.amount!)`, `direction: t.amount! < 0 ? "credit" : (t.direction ?? "debit")`, `model_kind: t.kind`, `raw_line: t.raw_line`. Write the `printed_*` totals onto `statements` and set `period_inferred = (parsed.period_start == null)` **in the update at `:69-76`, which must also be moved to run AFTER the confirmed-match guard at `:78-101`** — today it writes the period before deciding whether to keep the existing transactions.
- `app/api/statements/parse/route.ts:130` — `{ok:true, count:0}` becomes `{ok:false, message:"No transactions were found in this file."}`; `components/statements/statement-uploader.tsx:152-155` currently renders zero rows as a green badge.
- `app/api/statements/parse/route.ts:60-65` — the card lookup is `.eq("last4", parsed.card_last4)` with **no `user_id` scope**; add it, and add the `/^\d{4}$/` guard that currently only exists at `:106-108`.
- `app/api/statements/parse/route.ts:104,121-128` — chunk the insert (500 rows) and derive `period_start`/`period_end` from `min/max(txn_date)` after insert when the parser returned null.

**New:**
- `lib/reconciliation/noise.ts` — `noiseKeyOf()`, the TypeScript mirror of `noise_key_of()`, plus a test asserting the two agree. (This mirroring is already documented practice at `0015:22` for `normalizeVendor`.)
- `setLineKind(chargeId, kind, note?)` in `lib/reconciliation/actions.ts` — three things: fix this charge (`kind_source='user'`, `fee_auto_flagged=false`); upsert a `learning_rules` row `{rule_type:'line_kind', pattern: noiseKeyOf(description), action:{line_kind, direction}}` so every future statement carrying that line is classified on ingest; and sweep history now with `.eq("noise_key", key).neq("kind_source","user")` using the generated column from §2.2 F. **`kind_source='user'` is never overwritten by a rule or by re-parsing.**
- UI: the two collapsed strips from Step 7 gain per-row **"Actually needs a receipt"**; every open purchase row gains a small kind picker behind `···` (*Bank fee · Interest · Payment I made · Refund · Not a transaction*). `kind_source` drives a badge: `pattern`/`model` → *auto*, `rule` → *"because you told me before"*, `user` → *"you set this"*. This is `fee_auto_flagged`'s original intent (`0015:51`, *"so a wrong auto-flag stays visible"*) made visible at last.
- The completeness line in the header becomes real:
...
**Verify:** re-parse one statement; the `OVERLIMIT FEE` line is present and classified rather than absent. Mark one line as a bank fee; confirm it disappears from the list, appears in the strip, and that a second statement carrying the same line is classified on ingest.

---

## Step 12 — The pairing board at `/reconcile/board`

Full specification in **§4.5**. **Ship order inside the step is not negotiable** — the three sub-steps are independently useful and independently revertible:

1. **12a — the floor.** Every charge row on `/reconcile` keeps a `<details>` containing the existing `<AttachReceipt>` (`components/matching/attach-receipt.tsx:19-157`) — a real `<form action={attachReceiptToCharge}>` with a radio list, sorted by closeness to the line amount (`:46-53`) with exact matches flagged (`:103`). Works with JavaScript off.
2. **12b — tap-to-hold + keyboard + optimistic cross-off.** This is the complete feature on Andrew's phone: tap a charge, the opposite list re-sorts closest-amount-first with exact matches pinned under a green **EXACT** pill and the signed delta printed on near misses, tap a receipt, done. Two taps, no precision required.
...
**Refusing to drag on touch removes the entire scroll-versus-drag arbitration problem** — no `touch-action: none` on the lists, no long-press timer competing with iOS text selection, no `pointercancel` recovery. That is the highest-defect-density surface in this plan and it is deleted rather than mitigated, at zero functional cost.

**After this step Andrew can:** sit down at a desk and blitz the residue, or clear it two taps at a time on his phone.
**Verify:** on an actual iPhone, both lists scroll normally, tap-to-hold works, and a mis-tap is undoable from the toast for 8 seconds.
...
- **Bulk "Send these to the accountant"** on the orphan-receipt section — multi-select plus `setReceiptsSent` (`lib/receipts/actions.ts:259-268`). This is **27 receipts worth TTD 11,648.69**, the single largest pot in the dataset, and today the only way to act on it is to leave the close-out screen entirely. Add `revalidatePath("/reconcile")` to that action, which it currently lacks.
- **"Copy the chase list"** — plain text to the clipboard so Andrew can paste it into WhatsApp from his phone.

**Changed:**
- `lib/reports/append-receipts.ts` — add `APPENDIX_MAX_RECEIPTS` and a `deadline` parameter; route the `.in("receipt_id", ids)` at `:62-66` through `fetchAll`.

**After this step Andrew can:** hand the accountant one correct document for everything, and clear the biggest outstanding pile in two taps.

---

## Step 14 — Retire `/matching`, wire settings, clean up

- `app/(app)/matching/page.tsx` → permanent redirect to `/reconcile`. `runMatching` (`lib/matching/actions.ts:12-96`) is deleted; `attachReceiptToCharge` moves to `lib/reconciliation/actions.ts` (keeping a re-export so nothing breaks mid-flight).
- `app/api/reports/statement/route.ts` — the legacy per-statement PDF reads raw `statement_transactions` (`:49-53`) so its totals still double-count carried-over charges, and matches via `.in("statement_transaction_id", txnIds)` (`:63`) so it still reports charges covered elsewhere as missing. Either point it at `charge_reconciliation` or remove it.
- `components/settings/settings-form.tsx:14` and `lib/settings/actions.ts:36-40` — expose the five `0014` columns in plain English: *"Look for receipts up to [60] days before and [15] days after a charge"*, *"Treat statement lines within [4] days as the same charge"*, and one switch: *"Let a run tick things off by itself when it is certain"* (`auto_confirm_enabled`, default off). `lib/types.ts:37-45` `UserSettings` gains all seven new columns.
- Drop `rsm_backup_pre_0016`, `charges_backup_pre_0019`, `stmt_txn_backup_pre_0019` **only after a full close-out cycle has been completed and Andrew confirms the numbers.**

---

## Step 15 — Edit a card (half a day, small and self-contained)

Cards can currently only be **created and deleted**. `lib/cards/actions.ts` exports `createCard` (`:11-46`) and `deleteCard` (`:48-55`) and nothing else, and `components/cards/card-form.tsx:6-10` hard-binds `createCard` via `useActionState`. So fixing a typo in a nickname, correcting the last 4 digits, or changing a card from personal to company means **deleting the card and adding it again** — which silently drops the `cards.id` that `receipts.card_id` and `statements.card_id` point at (both are `ON DELETE SET NULL`, `0001_initial_schema.sql:145,185`).

**Changed:**
- `lib/cards/actions.ts` — add `updateCard(_prev, formData)`. Same validation as `createCard:26-32` (nickname required; `last4` either blank or exactly 4 digits; `card_type` in `CARD_TYPES`), then `.update({...}).eq("id", id)`. RLS scopes it to the owner, so no extra user check beyond the existing `getUser()` guard. Return the same `CardFormState` shape.
- `components/cards/card-form.tsx` — take an optional `card?: Card` prop. When present, bind `updateCard` instead of `createCard`, prefill every field via `defaultValue`, render a hidden `id`, change the heading to *"Edit card"* and the button to *"Save changes"*, and **skip the reset-on-success effect at `:14-17`** (resetting an edit form blanks the values the user just saved).
- `app/(app)/cards/page.tsx` — add an **Edit** control beside `DeleteButton` in each `<li>` (`:79-84`). Use a `?edit=<id>` search param that renders the form in edit mode at the top of the page rather than a new route: it is one file, it keeps the page a server component, and it works by tap on a phone.

**The part that is easy to get wrong — editing a card does NOT re-classify existing receipts.**

`classify.ts:69-89` resolves `last4 -> card -> payment_method` **once, at extraction time**, and writes the result onto the receipt. Nothing recomputes it later. Two consequences the edit screen must handle rather than hide:

1. **Changing `card_type`** (e.g. personal to company) changes how *future* receipts classify, but every existing receipt keeps its stored `payment_method` and its derived `reimbursable` (`classify.ts:98`). Because `expected_on_statement` in `orphan_receipts` (`0019_orphan_receipts_scope.sql:36-39`) is computed from exactly those two fields, past receipts stay on whichever side of the close-out list they were already on.
2. **Changing `last4`** leaves a stale `learning_rules` row of type `last4_card` still pointing the **old** four digits at this card (written by `lib/receipts/actions.ts:126-135`). On the next upload that rule wins at `classify.ts:74-81`, so the old number keeps resolving to this card forever.

So `updateCard` must, in the same action: update the `last4_card` learning rule to the new pattern (or delete the row when `last4` is cleared), and the form must carry a plain-English note — *"This changes how new receipts are classified. Receipts already processed keep the payment type they were given; open one and re-save it to change it."*

**After this step Andrew can:** correct a card's nickname, last 4 digits, type or notes in place, without deleting it and breaking the link to receipts and statements already classified to it.
**Verify:** edit the nickname of First Citizens Mastercard `2726` — the receipts already linked to it still show it. Change a card's `last4` and confirm the old `last4_card` rule is gone from `learning_rules`. Clear a `last4` entirely and confirm the form accepts it (the column is nullable, `0001:96`).
...
Verified against the real corpus (64 charges, 86 receipts) with a working prototype run over the **full 928-pair cross product**. Everything here is measured.

### 4.1.1 What the live descriptions actually look like

The brief's five examples understate the problem:
...
Two consequences: a Trinidad geo gazetteer is **not** needed — legal suffixes and store numbers mark the boundary far more reliably; and `CAL_CORP_CTO` is the real "SCL is SCL" case in this dataset, and it is deterministically unsolvable.
...
Token classes, all closed lists, deliberately minimal — every word removed is discrimination lost:
...
`CHANNEL` is three words on purpose. `web`, `store`, `online` and `bill` are **not** in it — `WEB SOURCE` is a real merchant and `MASSY STORES` must stay distinguishable from `MASSY MOTORS`. `TLDISH` is a regex, not a list, so it handles `ANTHROPIC.COMCA` → `comca` and `Amazon.com` → `com`.
...
**IDF corpus** — built once per run over **distinct zone strings ∪ distinct receipt vendor keys**, *not* over the 90 statement lines. Deduping is essential: `PEAKE PETROLEUM` appears on 15 lines but must count as one document. Measured on live data: N = 49, `idf(trinidad)` = 2.97 (df 6), `idf(peake)` = 3.12 (df 5), `idf(scl)` = 3.81 (df 2). Cost is microseconds. Because scores are a normalised ratio they are far more drift-stable than raw IDF as the corpus grows, and confirmed matches are immutable, so drift only affects future suggestions.

**Thresholds:** `ALIAS_STRONG = 0.75` (merchant identified) · `MIN_VENDOR_EVIDENCE = 0.45` (below this, no merchant evidence at all).
...
**Alias match is containment, not equality**: an alias fires when `vendor_tokens ⊆ receiptTokens(candidate.vendor_name)`. Equality would break the moment a receipt reads `Caribbean Airlines Limited (CAL)`.
...
**Write sites — every one is a human asserting "this description is this vendor":** `attachReceiptToCharge` (`lib/matching/actions.ts:105-177`, after the upsert at `:160-172`; it already selects `description` at `:121`, so no extra query); `confirmMatch` (`:179-195`, needs one added join for `description` + `vendor_name`); and every drag/tap drop, which routes through the same action.
...
2. **Only learn what was got wrong** (`score < ALIAS_STRONG`). On the live corpus that means 23 of the 24 strong pairs write nothing — the table stays small and every row is meaningful.
...
**Negative aliases are not needed.** A rejection is per-pair evidence and `rejected_at` (`0016:19`) is already the right home at the right granularity. Do not invent a second mechanism.
...
"Needs a receipt" and "counts as money" are orthogonal questions.
...
Every exactness affordance in the app — pre-pass, pill, ring, sort — routes through this one function. **Every confirmed USD match also writes back the implied observed rate** (`ttd_amount_charged / amount`) into `receipts.notes` or a run summary, so Andrew can see the drift.
...
- **A per-receipt flag (`receipts.last_matched_at`) — rejected.** The unit of work is a *pair*; the flag knows one side. `assign_charge()` creates a new charge the moment any new statement line lands, and every one of those must be scored against every existing receipt. To be correct the predicate degenerates to `receipt.last_matched_at > (select max(created_at) from charges)` — a full rescan wearing a flag. It also cannot express a negative, and `receipts.updated_at` cannot substitute (see §2.2 Section C).
...
**Deliberately NOT stored: scores that fell below threshold.** 64 charges × 82 receipts ≈ 5,200 `scorePair` calls is single-digit milliseconds. The expensive thing is not CPU, it is re-showing Andrew a suggestion he already dismissed — and that is solved by rejection durability, not by caching. Caching would also need an `ALGO_VERSION` and a re-score migration, because the scorer is about to be rewritten. **Full recompute has no version to bump.**
...
**The live bug this replaces:** `lib/matching/actions.ts:65-69` is `.delete().in("statement_transaction_id", txnIds).eq("confirmed", false)`. **A rejected row has `confirmed = false`.** Fixing `rejectMatch` alone would leave the next run deleting the rejection anyway. Both must ship together (Step 10).
...
**A new statement introduces charges that old receipts now match.** Found automatically, because the receipt input set is never narrowed by time-of-run. The one thing that could narrow it wrongly is the candidate window, made safe by three rules: it is anchored to the **charge's** `anchor_date`, never a statement period or a run timestamp; confirmed and sent receipts are exempt (`0014:25-26`); and it is **visible** — the run reports how many receipts fell outside it, and manual attach and the board apply **no window at all**.
...
| **Step 6's `?all=1` / `?state=` / `?rp=` parameter scheme** on `/matching` | Replaced by `/reconcile` with filter chips over an outstanding-first model. `/matching` is retired at Step 14. |
| **The 4-value `ChargeState`** at `plan:662` and `plan:483-489` | Wrong. The applied view has **five** states (`0017:73-79`) and this plan adds `is_open` alongside. |
...
- **A `reconciliation_runs` table with run-history and run-result screens.** Its own design concedes the watermark is advisory and correctness never depends on it. `decided_via` plus two scalars on `user_settings` deliver attribution, the "auto-closed this run" review tray, per-item undo and the NEW-since lens with zero new tables and zero new screens. Run status pills, red failed-run rows and a batch-undo button are an operator console; Andrew is one non-technical owner on a phone.
...
- **Prefix matching as sufficient evidence.** `star` prefix-matches `starbucks`. It is used one-way (§4.1.3 rule b) and as a tiebreaker only.
- **Feeding aliases into `charge_key_of`.** §4.1.5 guard 4. This would silently merge genuinely different charges.
...
|---|---|---|
| **Reclassification of all 64 charges and 90 lines** — `line_kind`, `direction`, `kind_source`, and **`no_receipt_expected` can be both SET and UNSET** | `0019` Section F | `charges_backup_pre_0019` and `stmt_txn_backup_pre_0019` (Section A). `update charges c set no_receipt_expected = b.no_receipt_expected, fee_auto_flagged = b.fee_auto_flagged from charges_backup_pre_0019 b where b.id = c.id;` |
| **Auto-closing fee/interest/payment/summary charges** (`closed_at`, `closed_by='auto'`, `closed_reason`) | `0019` Section F | Same backup; or `update charges set closed_at=null, closed_by=null, closed_reason=null where closed_by='auto';` — and every one is visible in the "Auto-closed since you last looked" tray with a one-tap Reopen. |
| **`amount >= 0` check on `statement_transactions`** | `0019` Section E | Fails loudly on apply if any negative exists (there are none in live data). `alter table … drop constraint stmt_txn_amount_nonneg;` |
| **`state` CASE reorder in `charge_reconciliation`** | `0019` Section H | `create or replace view` with the `0017:73-79` body. Affects zero rows in current live data — nothing sets `no_receipt_expected` except the fee flag and none of those carry receipts. |
| **`orphan_receipts` widened to `needs_review`** | `0019` Section H | Same. It only *adds* rows; the `needs_review` badge marks them. |
| **`drop view reconciliation_months`** | `0019` Section H | Re-run the `create or replace view` from `0017:126-146`. Zero consumers, verified. |
| **`receipts` table rewrite** for the STORED generated column | `0019` Section C | `alter table receipts drop column match_fingerprint;` 86 rows — instant. |
| **`rejectMatch` stops deleting** | Step 10 code | Behavioural, not destructive: it now writes where it previously destroyed. Reversal is a code revert. |
| **`deleteStatement` gains a guard** | Step 5 code | Code revert. Note that today's version is the dangerous one. |
...
3. **Nothing currently defines completeness against the paper statement.** `TxnSchema` carries no raw line text and no per-line confidence; `app/api/statements/parse/route.ts:130` returns `{ok:true, count:0}` on a zero-transaction parse, which `components/statements/statement-uploader.tsx:152-155` renders as a **green badge**. So "everything crossed off" could mean "the parser found 12 of 34 lines and you crossed off all 12." The `printed_*` totals in §2.2 Section I and the completeness line in Step 11 are the only real check that will exist, and until they land the header must say so.
4. **Two statements parsed with `period_start IS NULL`** and `0014:33-40` backfilled them without recording that it had. Any header or report printing a period must read **"dates inferred from transactions"** unless it was genuinely read off the document (§2.2 Section I, Step 13).
